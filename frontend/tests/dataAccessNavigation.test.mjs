...
test("data access development workbench combines data sources and datasets", () => {
  ...
  assert.match(view, /class="page-tabbar development-tabbar"/)
  assert.match(view, /class="page-tab page-tab--stacked"/)
  assert.doesNotMatch(view, /<el-tabs/)
...
test("connector access page is named as connector entry instead of generic data access", () => {
  ...
  assert.match(view, /连接器接入/)
  assert.match(view, /外部业务系统同步入口/)
  assert.match(view, /ERP、WMS、CRM、SaaS API/)
  assert.match(view, /连接器接入负责把外部系统数据拉进平台/)
  assert.match(view, /Connection/)
